...
function averageWordLength(words) {
  ...
  /*  expect(received).toBeNull()
    ...
    Received: NaN
      ...
      37 |        const words = ["eclipse", "harmony", "cascade"];
      38 |        expect(averageWordLength(words)).toBeCloseTo(7, 0);
    > 39 |        expect(averageWordLength([])).toBeNull();
         |                                      ^
      40 |      }); */
  if (words.length === 0) {
    return null;
...
function doesWordExist(haystack, needle) {
  // TODO:
  let isHaystack = wordsFind.includes(haystack) ? true : false;
  let isNeedle = wordsFind.includes(needle) ? true : false;
  console.log(isHaystack);
  console.log(isNeedle);
  if (isHaystack) return true;
  if (isNeedle) return true;
  return isHaystack && isNeedle;
}
// Count repetition
...
console.log(howManyTimes("matter", "machine"));
